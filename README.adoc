...
A premium, high-performance, pure-Python semantic parser, extractor, and translator for Python docstrings written in AsciiDoc. Fully compatible with Python 3.14+ and WASM/Pyodide environments with zero native compiled extensions.
...
* `AsciiDocStringParseError` +
  Raised when parsing of an AsciiDoc docstring fails. Inherits from `ValueError`.
